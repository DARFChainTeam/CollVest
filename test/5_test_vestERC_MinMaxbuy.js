...
let snapshotId;
  ...
  it('should send amount1 of token1 to  vesting contract', async () => {
    ...
    try { //amount  < minBuy 
      await vestContract.putVesting(startVestConf.vest1.token1, accounts[2], 5, {from:accounts[2]} )
    ...
    try { //amount  > maxBuy 
      await vestContract.putVesting(startVestConf.vest1.token1, accounts[1], 500, {from:accounts[1], value:500} )
  ...
  it('Pause and try to  withdraw then paused (don`t have vested enough) ', async () => {
      ...
      try {
        await vestContract. pauseWithdraw("SCAM!") ;
      }  catch (e) {
        // console.log(e)
  ...
  it('withdraw t1 once approved amount 2nd period time', async () => {
    ...
    let av2claimt1 =  (await vestContract.availableClaimToken1()).toNumber();
    assert.equal (av2claimt1, startVestConf.vest1.amount1 * 2 /periods, "amount t1 2nd month ")
  ...
  it('withdraw t2 once approved amount 2nd period time', async () => {
   ...
   // assert.equal (av2claimt2, Math.round(startVestConf.vest1.amount2*2 /periods * vested1 / startVestConf.vest1.amount1), "amount t2 1st month ")
    assert.equal (balt2after1 - balt2before1,  av2claimt2, "balt1before1+ av2claimt1" );
